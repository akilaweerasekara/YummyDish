...
.acc-tab.active{background:var(--c-surface);color:var(--copper);box-shadow:var(--shadow);}
...
.loc-chip:hover{border-color:var(--copper);background:var(--copper-l);}
...
<div class="kg-page" style="padding-bottom:40px;max-width:680px;margin:0 auto;">
  ...
  <div class="acc-hero">
    <div style="display:flex;align-items:center;gap:16px;position:relative;z-index:1;margin-bottom:22px;">
      ...
      <div style="color:white;">
        <div style="font-family:var(--font-serif);font-size:1.4rem;font-weight:700;"><c:out value="${user.name}"/></div>
  ...
  <div style="padding:0 16px;margin-top:-20px;position:relative;z-index:10;">
    ...
    <!-- Loyalty card -->
    <div class="kg-card mb-3" style="border:2px solid rgba(255,184,0,.3);">
      <div class="kg-card-body" style="padding:18px 20px;">
    ...
    <div class="acc-pane active" id="pane-profile">
        ...
        <div class="kg-card-body">
          ...
          <div id="profileView">
            ...
            <div class="d-flex gap-2 mt-4">
              <button onclick="toggleEdit(true)" class="kg-btn kg-btn-primary" style="flex:1;"><i class="bi bi-pencil-fill me-2"></i>Edit Profile</button>
              <button onclick="showModal('pwdModal')" class="kg-btn kg-btn-outline" style="padding:10px 14px;"><i class="bi bi-key me-1"></i>Password</button>
          ...
          <div id="profileEdit" style="display:none;">
            <form action="/account/update" method="post">
              <div class="row g-3">
                <div class="col-12 kg-form-group"><label class="kg-label">Full Name</label><input type="text" name="name" class="kg-input" value="${user.name}" required></div>
                <div class="col-6 kg-form-group"><label class="kg-label">Phone</label><input type="text" name="phone" class="kg-input" value="${user.phone}"></div>
                <div class="col-6 kg-form-group"><label class="kg-label">Email (locked)</label><input class="kg-input" value="${user.email}" disabled style="opacity:.55;"></div>
                <div class="col-12 kg-form-group">
                  <label class="kg-label">Delivery Address</label>
                  <input type="text" name="address" id="addrInput" class="kg-input" value="${user.address}" placeholder="Kandy area">
                  <button type="button" onclick="fillGPS()" class="kg-btn kg-btn-outline kg-btn-sm mt-2" style="width:auto;padding:6px 14px;"><i class="bi bi-crosshair me-1" style="color:var(--copper);"></i>Use GPS</button>
                  <div id="addrMsg" style="font-size:.75rem;color:var(--c-muted);margin-top:4px;"></div>
                </div>
                <div class="col-12"><hr style="border-color:var(--c-border);"><p class="kg-label mb-3">Payment Card (optional)</p></div>
                <div class="col-12 kg-form-group"><label class="kg-label">Card Number</label><input type="text" name="cardNumber" class="kg-input" placeholder="Leave blank to keep" maxlength="19" oninput="fmtCard(this)"></div>
                <div class="col-6 kg-form-group"><label class="kg-label">Cardholder</label><input type="text" name="cardHolder" class="kg-input" value="${user.cardHolder}"></div>
                <div class="col-6 kg-form-group"><label class="kg-label">Expiry MM/YY</label><input type="text" name="cardExpiry" class="kg-input" value="${user.cardExpiry}" placeholder="12/26" maxlength="5"></div>
              </div>
              <div class="d-flex gap-2 mt-3">
                <button type="submit" class="kg-btn kg-btn-primary" style="flex:1;">✅ Save Changes</button>
                <button type="button" onclick="toggleEdit(false)" class="kg-btn kg-btn-outline" style="color:var(--c-muted);">Cancel</button>
              </div>
            </form>
          </div>
        </div>
          ...
          <c:choose>
            ...
            <c:otherwise>
              <div style="text-align:center;padding:20px;color:var(--c-muted);">
                ...
                <button onclick="toggleEdit(true)" class="kg-btn kg-btn-primary kg-btn-sm" style="width:auto;padding:8px 20px;">Add Card</button>
              </div>
            </c:otherwise>
          </c:choose>
        </div>
      ...
      <!-- Sign out -->
      <a href="/logout" class="kg-btn kg-btn-outline d-flex justify-content-center mb-2 kg-fade" style="color:var(--c-muted);">
      ...
      <div class="kg-card mb-3">
        <div class="kg-card-body">
          <div class="row g-2 mb-4" id="orderStatsRow">
            <div class="col-4"><div style="background:var(--c-bg);border-radius:12px;padding:14px;text-align:center;border:1px solid var(--c-border);"><div style="font-size:1.6rem;font-weight:800;color:var(--copper);" id="stOrders">&mdash;</div><div style="font-size:.72rem;color:var(--c-muted);">Orders</div></div></div>
            <div class="col-4"><div style="background:var(--c-bg);border-radius:12px;padding:14px;text-align:center;border:1px solid var(--c-border);"><div style="font-size:1rem;font-weight:800;color:var(--copper);" id="stSpend">&mdash;</div><div style="font-size:.72rem;color:var(--c-muted);">Total Spent</div></div></div>
            <div class="col-4"><div style="background:var(--c-bg);border-radius:12px;padding:14px;text-align:center;border:1px solid var(--c-border);"><div style="font-size:1.6rem;font-weight:800;color:var(--leaf-green);" id="stDone">&mdash;</div><div style="font-size:.72rem;color:var(--c-muted);">Delivered</div></div></div>
          </div>
          ...
          <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <a href="/activity" class="kg-btn kg-btn-outline kg-btn-sm" style="width:auto;padding:6px 14px;font-size:.78rem;">View All</a>
          </div>
    ...
    <div class="acc-pane" id="pane-places">
      <div class="kg-card mb-3">
        <div class="kg-card-body">
          ...
          <div style="margin-top:12px;padding:12px 14px;background:var(--copper-l);border-radius:12px;border:1px solid rgba(255,107,53,.18);">
            <p style="font-size:.8rem;color:var(--copper);margin:0;"><i class="bi bi-lightbulb me-1"></i>Go to Menu → click your address → save as "Home", "Work", etc.</p>
          </div>
        </div>
      </div>
    </div>
...
<div id="delModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:9999;align-items:center;justify-content:center;padding:16px;">
  <div style="background:var(--c-surface);border-radius:24px;padding:32px;max-width:340px;width:100%;text-align:center;animation:scaleIn .3s var(--ease);">
    ...
    <h4 style="font-family:var(--font-serif);">Delete Account?</h4>
    ...
    <div class="d-flex gap-2">
      <form action="/account/delete" method="post" style="flex:1;"><button type="submit" class="kg-btn" style="width:100%;background:#f44336;color:white;padding:12px;">Delete Forever</button></form>
      <button onclick="hideModal('delModal')" class="kg-btn kg-btn-outline" style="flex:1;padding:12px;color:var(--c-muted);">Cancel</button>
    </div>
  </div>
</div>
...
<div id="pwdModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:9999;align-items:center;justify-content:center;padding:16px;">
  <div style="background:var(--c-surface);border-radius:24px;padding:32px;max-width:400px;width:100%;animation:scaleIn .3s var(--ease);">
    <h4 style="font-family:var(--font-serif);margin-bottom:20px;">🔑 Change Password</h4>
    <div id="pwdAlert"></div>
    <div class="kg-form-group"><label class="kg-label">Current Password</label><input type="password" id="cp1" class="kg-input" placeholder="Current password"></div>
    <div class="kg-form-group"><label class="kg-label">New Password</label><input type="password" id="cp2" class="kg-input" placeholder="Min 6 characters"></div>
    <div class="kg-form-group"><label class="kg-label">Confirm New Password</label><input type="password" id="cp3" class="kg-input" placeholder="Repeat new password"></div>
    <div class="d-flex gap-2">
      <button onclick="doChangePwd()" class="kg-btn kg-btn-primary" style="flex:1;">Update Password</button>
      <button onclick="hideModal('pwdModal')" class="kg-btn kg-btn-outline" style="color:var(--c-muted);">Cancel</button>
    </div>
  </div>
</div>
...
function fillGPS() {
  ...
  navigator.geolocation.getCurrentPosition(function(pos) {
    YDMaps.reverseGeocode(pos.coords.latitude, pos.coords.longitude, function(addr) {
      ...
      msg.style.color = 'var(--leaf-green)';
    });
...
function loadOrders() {
  ...
  fetch('/api/my-orders?limit=30')
    ...
    .then(function(orders) {
      if (!orders.length) {
        document.getElementById('ordersList').innerHTML = '<div style="text-align:center;padding:24px;color:var(--c-muted);">No orders yet. <a href="/menu" style="color:var(--copper);">Order now!</a></div>';
        return;
      }
      ...
      document.getElementById('ordersList').innerHTML = orders.slice(0,10).map(function(o) {
        ...
        return '<div class="order-hist-row">'
          ...
          +'<div style="font-size:.85rem;font-weight:700;color:var(--copper);">LKR '+Math.round(o.totalAmount).toLocaleString()+'</div>'
          +'</div>'
          +'<button onclick="reorder(\''+o.orderId+'\')" class="kg-btn kg-btn-sm" style="background:var(--copper-l);color:var(--copper);border:none;padding:6px 12px;font-size:.72rem;white-space:nowrap;margin-left:6px;">🔄 Again</button>'
          +'</div>';
      }).join('');
    });
}
...
function loadFavs() {
  ...
  c.innerHTML = favs.map(function(f) {
    ...
    return '<div class="fav-row">'
      ...
      +' class="kg-btn kg-btn-primary kg-btn-sm" style="white-space:nowrap;width:auto;">Add to Cart</button>'
      +'</div>';
  }).join('');
}
