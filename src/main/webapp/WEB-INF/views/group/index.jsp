...
.room-code-display{font-size:3.2rem;font-weight:800;letter-spacing:10px;font-family:monospace;color:var(--copper);text-align:center;padding:24px 20px;background:var(--copper-l);border-radius:16px;border:2px dashed rgba(255,107,53,.4);animation:pulse 2.5s ease-in-out infinite;}
...
.tab-btn.active{background:var(--c-surface);color:var(--copper);box-shadow:var(--shadow);}
</style>
...
<div class="kg-page">
  ...
  <div class="group-hero">
    <div class="container" style="position:relative;z-index:1;max-width:720px;text-align:center;">
      ...
      <h1 style="font-family:var(--font-serif);color:white;font-size:2.4rem;margin-bottom:10px;">Group Orders</h1>
  ...
  <div class="container py-4" style="max-width:720px;">
    ...
    <!-- CREATE -->
    <div id="paneCreate" class="kg-fade kg-visible">
        ...
        <div class="kg-card-body">
          ...
          <c:if test="${not user.hasCard()}">
            <div style="background:#FFF8E1;border:1px solid #FFE082;border-radius:12px;padding:14px 16px;margin-bottom:18px;font-size:.875rem;">
              ...
              Group orders require a saved card. <a href="/account" style="color:var(--copper);font-weight:700;">Add card →</a>
            </div>
          </c:if>
          <button onclick="createRoom()" class="kg-btn kg-btn-primary" id="createBtn"
      ...
      <div id="roomCreated" style="display:none;" class="kg-card mb-3">
        <div class="kg-card-body" style="text-align:center;">
          ...
          <p style="color:var(--c-muted);font-size:.82rem;margin:14px 0 18px;">Share this code with your team. They can join at <strong>katagasma.lk/group</strong></p>
          <div class="d-flex gap-2 justify-content-center">
            <button onclick="copyCode()" class="kg-btn kg-btn-outline kg-btn-sm" style="width:auto;"><i class="bi bi-clipboard me-1"></i>Copy Code</button>
    ...
    <!-- JOIN -->
    <div id="paneJoin" style="display:none;" class="kg-fade">
      <div class="kg-card mb-3">
        <div class="kg-card-body">
          <h5 class="fw-bold mb-3">Join a Room</h5>
          <div class="kg-form-group">
            <label class="kg-label">Room Code</label>
            <input type="text" id="joinCode" class="kg-input"
          ...
          <button onclick="joinRoom()" class="kg-btn kg-btn-primary" id="joinBtn">
      ...
      <div id="joinedRoom" style="display:none;" class="kg-card mb-3">
        <div class="kg-card-body">
          <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <span class="kg-pill kg-pill-ready" style="font-size:.72rem;">Active</span>
          </div>
          ...
          <div id="groupFoodList" style="max-height:280px;overflow-y:auto;">
            <div class="kg-skeleton" style="height:64px;border-radius:12px;margin-bottom:8px;"></div>
            <div class="kg-skeleton" style="height:64px;border-radius:12px;margin-bottom:8px;"></div>
            <div class="kg-skeleton" style="height:64px;border-radius:12px;"></div>
          </div>
          ...
          <div class="d-flex gap-2 mt-3">
            <button onclick="addGroupItemsToCart()" class="kg-btn kg-btn-primary" style="flex:1;">
    ...
    <!-- MY ROOMS -->
    <div id="paneMy" style="display:none;" class="kg-fade">
      <div id="myRoomsList">
        <div class="kg-skeleton" style="height:80px;border-radius:14px;margin-bottom:10px;"></div>
        <div class="kg-skeleton" style="height:80px;border-radius:14px;"></div>
      </div>
    </div>
...
function shareWhatsApp() {
  ...
  var text = '🍽️ *Katagasma Group Order*\nJoin my room with code: *' + code + '*\n\nGo to katagasma.lk/group and enter the code to add your food order!';
  ...
  try {
    ...
    c.innerHTML = avail.map(function(f) {
      return '<div class="food-pick">'
        ...
        + '<div style="font-size:.78rem;color:var(--copper);">LKR ' + Math.round(f.price).toLocaleString() + '</div>'
        + '</div>'
        + '<button onclick="addGroupItem(\'' + f.id + '\',\'' + f.name.replace(/'/g,'') + '\',' + f.price + ',\''+(f.imageUrl||'')+'\')" '
        + 'style="background:var(--copper-l);color:var(--copper);border:1px solid rgba(255,107,53,.3);border-radius:8px;padding:5px 12px;font-size:.78rem;font-weight:700;cursor:pointer;transition:all .2s;" '
        + 'onmouseover="this.style.background=\'var(--copper)\';this.style.color=\'white\'" '
        + 'onmouseout="this.style.background=\'var(--copper-l)\';this.style.color=\'var(--copper)\'">Add</button>'
        + '</div>';
    }).join('');
  ...
  if (el) {
    el.innerHTML = myGroupSel.map(function(i){
      return '<div style="display:flex;justify-content:space-between;padding:3px 0;">'
        ...
        + '<span style="color:var(--copper);">LKR ' + Math.round(i.price * i.qty).toLocaleString() + '</span></div>';
    }).join('');
  }
...
function loadMyRooms() {
  ...
  c.innerHTML = rooms.map(function(r) {
    return '<div style="background:var(--c-surface);border:1px solid var(--c-border);border-radius:16px;padding:16px 18px;margin-bottom:10px;display:flex;align-items:center;gap:14px;">'
      + '<div style="width:44px;height:44px;border-radius:50%;background:' + (r.role==='host'?'linear-gradient(135deg,var(--copper),var(--copper-d))':'linear-gradient(135deg,#667eea,#764ba2)') + ';color:white;display:flex;align-items:center;justify-content:center;font-size:1.2rem;flex-shrink:0;">'+(r.role==='host'?'👑':'👤')+'</div>'
      + '<div style="flex:1;">'
      ...
      + 'class="kg-btn kg-btn-outline kg-btn-sm" style="width:auto;">Rejoin</button>'
      + '</div>';
  }).join('');
}
</script>
